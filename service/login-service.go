...
func (s *LoginService) LoginGoogleAdmin(idToken string) (map[string]interface{}, error) {
	ctx := context.Background()
	now := time.Now().Format("2006-01-02 15:04:05")
	...
	// find user by email
	admin, err := s.UserRepo.GetAdminByEmail(email)
	if err != nil {
		fmt.Printf("[WARN] [%s] User %s tidak ditemukan di database. Harus registrasi dulu.\n", now, email)
		return nil, errors.New("user not found, please register first")
	}
	if admin.GoogleUID == "" {
		err := s.UserRepo.UpdateAdminGoogleUID(admin.ID, token.UID)
		if err != nil {
			return nil, err
		}
	} else if admin.GoogleUID != token.UID {
		// 🚨 TAMBAHKAN INI: Cegah jika ada yang mencoba membajak email dengan UID berbeda
		fmt.Printf("[CRITICAL] [%s] UID mismatch untuk email %s! Kemungkinan percobaan pembajakan.\n", now, email)
		return nil, errors.New("kredensial keamanan tidak valid, harap hubungi super-admin")
	}
	// generate access token
	accessToken, err := s.JWTManager.GenerateAccessToken(admin.ID, admin.Email, admin.Role)
	if err != nil {
		fmt.Printf("[ERROR] [%s] Gagal generate Access Token untuk %s: %v\n", now, email, err)
		return nil, err
	}
	...
	// generate refresh token
	refreshToken, err := s.JWTManager.GenerateRefreshToken(admin.ID)
	if err != nil {
		fmt.Printf("[ERROR] [%s] Gagal generate Refresh Token untuk %s: %v\n", now, email, err)
		return nil, err
	}
	...
	tokenData := model.AdminRefreshToken{
		AdminID:   admin.ID,
		Token:     refreshTokenHash,
		ExpiresAt: expiresAt,
	}
	...
	// Upsert token login (Update jika sudah ada, Insert jika belum)
	err = s.RefreshRepo.UpsertTokenLoginAdmin(tokenData)
	if err != nil {
		fmt.Printf("[ERROR] [%s] Gagal simpan/update Refresh Token di DB: %v\n", now, err)
		return nil, err
	}
